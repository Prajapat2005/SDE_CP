...
int MCM_dp(vector<int> v, vector<vector<int>> dp, int i,int j){
   ...
   for(int p=0;p<v.size();p++) dp[p][p]=0;
    int mini = INT_MAX;

    for(int k=i;k<=j-1;k++){
        if(dp[i][k]==-1) dp[i][k] = MCM_dp(v,dp,i,k) ;
        if(dp[k+1][j]==-1) dp[k+1][j] = MCM_dp(v,dp,k+1,j);
...
int MCM_dp_1(vector<int> v, vector<vector<int>> dp){
    ...
    int n=v.size();
   for(int p=1;p<v.size();p++) dp[p][p]=0;  // base case i==j return 0;
